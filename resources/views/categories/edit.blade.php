                ...
                <form action="{{ route('categories.update', $category) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center mt-1">
                            <input type="color" name="color" id="color" value="{{ old('color', $category->color) }}" class="h-10 w-10 rounded">
                            <input type="text" name="color_display" id="color_display" value="{{ old('color', $category->color) }}" class="ml-2 block rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" readonly>
                        </div>
                        @error('color')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
